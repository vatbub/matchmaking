...
class ServerServletTest : KotlinTestSuperclass() {
    ...
    @Test
    fun noResponseTest() {
        val response = doRequestNoTypeParam(DummyRequest(connectionId))
        assertExceptionResponse(InternalServerErrorException(), IllegalStateException("No response generated by server"), 500, response)
    }
    ...
    fun illegalArgumentExceptionTest() {
        ...
        val handler = DynamicRequestHandler({ true }, {
    ...
    fun internalServerErrorExceptionTest() {
        val expectedInnerException = java.lang.ArrayIndexOutOfBoundsException("Test exception")
        val handler = DynamicRequestHandler({ true }, {
            throw expectedInnerException
        })
        serverContext.messageDispatcher.registerHandler(handler)
